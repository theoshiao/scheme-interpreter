...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return procedure.eval_call(expr.second, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if len(expressions) == 0:
    	...
    	#used to have 'expressions.first =' in the line under
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        evaloperands = operands.map(lambda y: scheme_eval(y, env))
        return scheme_apply(self, evaloperands, env)
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        evaloperands = scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(evaloperands), env, True)
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
    	...
    	andeval = scheme_eval(expressions.first, env)
    	if scheme_falsep(andeval):
    		return False
    	elif len(expressions.second) == 0:
    		return andeval#(scheme_eval)
    	else:
    		return do_and_form(expressions.second, env)
    """
    if len(expressions) == 0:
    	return True
    while len(expressions.second) > 0:
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
    	...
    	oreval= scheme_eval(expressions.first, env)
    	#if scheme_truep(oreval) and len(expressions.second) == 0:
    		#return scheme_eval(expressions.first, env, True)
    	if scheme_truep(oreval):
    		return scheme_eval(expressions.first, env, True)
    	else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
    	pair = bindings.first
    	check_form(pair, 1, 2)
    	formals = Pair(pair.first, formals)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        env.bindings[target.first] = MacroProcedure(target.second, expressions.second, env)
        return target.first
        # END PROBLEM 10
    else:
        bad_target = target.first if isinstance(target, Pair) else target
